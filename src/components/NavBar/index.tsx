...
export default function NavBar() {
	...
	const whatsappNumber = '559991558807';
	...
	useEffect(() => {
		// Garante que a leitura só acontece no cliente
		setClientName(process.env.NEXT_PUBLIC_CLIENT_NAME || '');
	}, []);

	if (clientName === null) {
		// Evita renderização até termos o valor, prevenindo hidratação incorreta
		return null;
	}
		...
		<>
          ...
          >
            ...
            <span className="font-medium">Denuncie aqui</span>
				...
				<div
					onClick={() => {
						if (clientName === 'sincogra') {
							// const whatsappNumber = process.env.NEXT_PUBLIC_WHATSAPP_NUMBER; // ex: "5511999999999"
							if (whatsappNumber) {
								window.open(`https://wa.me/${whatsappNumber}`, '_blank');
							} else {
								console.error('Número do WhatsApp não definido no .env');
							}
						} else {
					...
					className={cn(
						...
						clientName === 'sincogra' ? 'bg-[#027cff8e]' : ''
					)}
				>
					<span className='font-medium'>
						{clientName === 'sincogra' ? 'Filiar-se' : 'Emita seu boleto'}
					</span>
				</div>
			</nav>
